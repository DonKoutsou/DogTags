...
//	protected SCR_InventorySlotUI	 							m_pStorageUIFrom;
	...
	//------------------------------------------------------------------------------------------------
	void ShowItemInfo( string sName = "", string sDescr = "", float sWeight = 0.0, SCR_InventoryUIInfo uiInfo = null, string CName = "", )
	{
		if ( !m_pItemInfo )
		{
	...
	protected void SetFocusedSlotEffects()
	{
		...
		if ( !itemInfo )
			...
			ShowItemInfo( itemInfo.GetName(), itemInfo.GetDescription(), invItemComp.GetTotalWeight(), SCR_InventoryUIInfo.Cast(itemInfo), cname);
